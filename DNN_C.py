...
def main():
    # load Data
    df = pd.read_hdf('data.h5', key='train_class', mode='r').dropna()

    # load validation date
    df_val = pd.read_hdf('data.h5', key='test_class', mode='r').dropna()
    ...
    # training
    history = model.fit(trX, trY, nb_epoch=1000, verbose=2, validation_data=(valX, valY))
